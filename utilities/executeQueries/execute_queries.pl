...
sub process_config_file {
    ...
    while(<CONFIG>) {
        ...
        if($data[0] =~ 'qd') {
            ...
            if($data[0] =~ 'create_required_directories') { $debug = $data[1]; $variable_check{'create_required_directories'} = 1; }
...
sub run_query {
    ...
    if($staggered_launch_window_secs > 0) {
        ...
        $elapsed_secs = ($num_simulation_ticks -
                          ($end_simulation_ticks - get_epoch_seconds()));
...
sub clean_up {
    ...
    if($insert_output) {
        ...
        my $copy_cmd = "\\COPY $output_tablename FROM \'$source_filename\' HEADER NULL AS \'NULL\'";
...
sub prepare_output_schema_and_table {
    ...
    if($insert_output) {
        ...
        my $table_create = "CREATE TABLE $output_tablename ( queryType integer,
                                   queryId varchar, queryRunId varchar,
                                   startTimestamp timestamp, startTimeEpoch numeric(19,9),
                                   endTimestamp timestamp, endTimeEpoch numeric(19,9),
                                   numRowsReturned integer, excelPivotHelper integer,
                                   Runtime numeric(8,4), launchFlag varchar(1),
                                   completeFlag varchar(1), successFlag integer,
                                   runName varchar, runNickname varchar )
